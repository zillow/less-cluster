...
utils.mix(LessCluster.prototype, Logger.prototype);
utils.mix(LessCluster.prototype, {
    ...
    **/
    ...
    /**
    For a given parent file, ensure that its dependents are
    mixed (if they exist) onto the collated memo object.
    ...
    @private
    **/
    _eachParent: function (memo, file) {
        var children = this._parents[file];
        if (children) {
            utils.mix(memo, children);
        }
    },

    /**
    For a given child file, ensure that its consumers are
    mixed (if they exist) onto the collated memo object.
    ...
    @private
    **/
    _eachChild: function (memo, file) {
        var eachParent = this._eachParent.bind(this, memo);
        var parents = this._children[file];

        if (parents) {
            utils.mix(memo, parents);
            Object.keys(parents).forEach(eachParent);
        } else {
            eachParent(file);
        }
    ...
    @private
    **/
    _filterAdHocFiles: function (adHocFiles) {
        var memo = {};

        adHocFiles.forEach(this._eachChild.bind(this, memo));
    ...
    **/
    _finishCollect: function (err, data) {
        ...
        if (this.options._files) {
            filesToProcess = this._filterAdHocFiles(this.options._files);
        }
